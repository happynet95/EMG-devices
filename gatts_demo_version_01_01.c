...
void Setup(){
    ...
    esp_bt_controller_config_t bt_gattc_cfg = BT_CONTROLLER_INIT_CONFIG_DEFAULT();

    ret = esp_bt_controller_init(&bt_gattc_cfg);
    if (ret) {
    ...
    esp_err_t local_mtu_ret_server = esp_ble_gatt_set_local_mtu(500);
    if (local_mtu_ret_server){
        ESP_LOGE(GATTS_TAG, "set local  MTU failed, error code = %x", local_mtu_ret_server);
        return;
    }
    // GATTS_DEMO Setup;
}
...
void app_main()
{
    ...
    EMG_RES_PACKET EMGData;

    EMGData.header = 4294967295;
    EMGData.count = 0;
    EMGData.add = 0;
    for(int i=0; i<10 ; i++){
        EMGData.muscle[i]=i+4;
        EMGData.add+=EMGData.muscle[i];
    }
    ...
    while(1){
        ...
        ESP_LOGI(GATTS_TAG,"void Loop!");
        vTaskDelay(100);
        printEMGvalue(EMGData);
        sendEMGvalue(EMGData);
        EMGData.count++;
    }
}
